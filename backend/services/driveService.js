 ...
 * @param {string} fileNamePattern - El patrón que debe contener el nombre del archivo (ej: "mphb-bookings").
...
async function findLatestFile(drive, folderId, fileNamePattern) {
  try {
    const res = await drive.files.list({
      ...
      pageSize: 1, // Solo nos interesa el más reciente
    });
 ...
 * @returns {Promise<string|Buffer>} El contenido del archivo.
 */
async function downloadFile(drive, fileId) {
  try {
    const response = await drive.files.get(
      { fileId: fileId, alt: 'media' },
      { responseType: 'stream' } // Descargamos como un stream de datos
    );
    return response.data; // Devolvemos el stream
  } catch (error) {
